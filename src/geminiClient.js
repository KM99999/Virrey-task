...
export async function generateLSG(query, intent, opts = {}) {
  ...
  if (seg === "mas_facil") {
    ...
    reteach = "\n\nIMPORTANTE: es un SEGUIMIENTO del MISMO tema (arriba aparece el Tema y el Mensaje del alumno). NO cambies de tema bajo ninguna circunstancia. Responde el MENSAJE del alumno DENTRO de ese tema: si pide un EJEMPLO o ANALOGÍA distinta (perritos, manzanas, dinero, deporte…), úsala manteniendo el tema y sin repetir el ejemplo anterior; si es una pregunta conceptual (\"¿eso quiere decir…?\", \"¿qué relación tienen… con…?\"), respóndela de forma clara y directa, con un ejemplo, y cierra con una pregunta de práctica del tema.";
  } else if (reexplain) {
